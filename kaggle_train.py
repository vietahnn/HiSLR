...
# Copy file này vào Kaggle notebook và chạy từng cell
# ─────────────────────────────────────────────────────────────────────────────
...
# ===========================================================================
# CELL 3 — Import HiSLR từ /kaggle/working/HiSLR
# ===========================================================================
# %%
import sys

# Code nằm tại /kaggle/working/HiSLR/hislr/
...
try:
    ...
    print("→ Kiểm tra lại CODE_DIR:", CODE_DIR)
    print("→ Cấu trúc cần có: /kaggle/working/HiSLR/hislr/__init__.py")
    ...
    for class_dir in sorted(split_dir.iterdir()):
        ...
        for video_file in sorted(class_dir.iterdir()):
            if video_file.suffix.lower() in {".mp4", ".avi", ".mov"}:
                ...
                if not out.exists():   # bỏ qua nếu đã extract rồi
...
# Thu thập class từ thư mục train
...
# Build annotation cho từng split
for split in ["train", "val", "test"]:
    split_dir = Path(DATA_ROOT) / split
    if not split_dir.exists():
        continue
    ...
    for class_dir in sorted(split_dir.iterdir()):
        ...
        for video_file in sorted(class_dir.iterdir()):
            ...
            samples.append({
                ...
                "phon_labels":   [-1] * 16,   # không có ASL-LEX → tắt PhAP tự động
            })

    out_path = f"{ANN_DIR}/{split}_annotations.json"
    with open(out_path, "w") as f:
        json.dump({"num_classes": num_classes, "class_names": class_names, "samples": samples}, f)
    print(f"  {split}: {len(samples)} samples → {out_path}")


# ===========================================================================
# CELL 6 — Cấu hình model (phù hợp với Kaggle GPU)
# ===========================================================================
# %%
...
cfg.data.data_root       = ANN_DIR                      # /kaggle/working/HiSLR/annotations
cfg.data.annotation_file = "train_annotations.json"
cfg.data.num_classes     = num_classes
cfg.data.num_frames      = 16
cfg.data.frame_size      = 224
cfg.data.num_workers     = 2                            # Kaggle giới hạn CPU

# ── Model ─────────────────────────────────────────────────────────────────────
cfg.model.swin_variant        = "swin_v2_t"             # nhỏ nhất, phù hợp Kaggle T4/P100
...
cfg.model.use_phap            = False                   # tắt vì không có ASL-LEX
cfg.model.use_tcr             = True

# ── Training ──────────────────────────────────────────────────────────────────
cfg.training.batch_size     = 8                         # giảm xuống 4 nếu bị OOM
cfg.training.stage1_epochs  = 30                        # rút ngắn cho Kaggle (giới hạn 12h)
...
cfg.training.use_amp        = True                      # mixed precision tiết kiệm VRAM
cfg.training.num_workers    = 2
cfg.training.checkpoint_dir = f"{OUTPUT_DIR}/checkpoints"   # /kaggle/working/HiSLR/outputs/checkpoints
cfg.training.log_dir        = f"{OUTPUT_DIR}/logs"          # /kaggle/working/HiSLR/outputs/logs
...
print(f"  log_dir         : {cfg.training.log_dir}")
print(f"  Model           : {cfg.model.swin_variant}  |  fusion_stages={cfg.model.fusion_stages}")
